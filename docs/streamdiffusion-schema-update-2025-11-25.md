...
- Four new "processor" blocks now exist on the pipeline params: `image_preprocessing`, `image_postprocessing`, `latent_preprocessing`, `latent_postprocessing`. Each block shares the same `ProcessingConfig` shape:
  ...
  ```84:118:runner/src/runner/live/pipelines/streamdiffusion/params.py
...
Refer back to [`StreamDiffusionParams`](https://github.com/livepeer/ai-runner/blob/main/runner/src/runner/live/pipelines/streamdiffusion/params.py) for the authoritative list of runtime-updateable attributes.
